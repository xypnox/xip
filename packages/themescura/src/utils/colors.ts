...
export const getShades = (color: Color, steps = [2, 2], shift: {
  chroma?: number;
  lightness?: number;
  hue?: number;
} = {
  ...
  const shades = Array.from({ length: totalSteps }).map((_, i) => i)
    .reduce((acc, _step, i) => {
      const isNegative = i < steps[0]
      const idx = isNegative ? i - steps[0] : i - steps[0] + 1
      const shifted = {
        lightness: shift.lightness !== undefined ? (isNegative ? shift.lightness : shift.lightness) * idx : undefined,
        chroma: shift.chroma !== undefined ? (isNegative ? shift.chroma : shift.chroma) * idx : undefined,
        hue: shift.hue !== undefined ? (isNegative ? shift.hue : shift.hue) * idx : undefined,
      }
